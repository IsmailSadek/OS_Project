                ...
                if (che)
                {
                    ...
                    { 
                        if(arguments.Count==2)
                        {
                            ...
                            {
                                c_dic.Directory_Table.Clear();
                                if (c_dic.Search_directory(d[^1])!=-1)
                                {
                            ...
                            else 
                            {
                                ...
                                if (c_dic.Search_directory(d[^1]) == -1)
                                {
                                    ...
                                    {
                                        c_dic.Directory_Table.Clear();
                                        int pos = c_dic.Search_directory(d[^1]);
                                        if (pos != -1)
                                        {
                                            string r = "";
                                            File_Entry f = new File_Entry(d[^1], 0, c_dic.Directory_Table[pos].firstCluster, r, c_dic);
                                            f.readFileContent();
                                            File_Entry nwf = new File_Entry(d[^1], 0, 0, f.content, old);
                                            nwf.writeFileContent();
                                            nwf.readFileContent();
                                            DirectoryEntry nwd = new DirectoryEntry(d[^1], 0, nwf.firstCluster, nwf.fileSize);
